...
/**
	\brief	Работа с аппаратным модулем через библиотеку BARDY

	Описание библиотеки приведено в документе "DCR0328 - Управляющая библиотека BARDY".

	Обращение к регистрам производится через службу REG0, в файле brd.ini требуется подключение devreg.

	Класс обеспечивает инициализацию модуля, обращения к регистрам тетрады, работу со стримами.
...
class  CL_AMBPEX
{
	...
	cl_ambpex_task_data	 *td;	//!< внутренние данные класса
 ...
 public:
       // virtual char* GetName( void );  //!< Возвращает название класса
		...
		int StreamSetTrd( U32 strm, U32 trd );
		...
		BRD_Handle GetSrvHandle( BRDCHAR* name, U32 *atr, U32 mode = BRDcapt_EXCLUSIVE );
		...
		void PkgCmdStart( void );

		//! Добавление команды записи в косвенный регистр
		void PkgCmdPokeInd( U32 trd, U32 reg, U32 val );

		//! Добавлние команды записи в прямой регистр
		void PkgCmdPokeDir( U32 trd, U32 reg, U32 val );

		//! Добавление команды чтения из косвенного регистра
		void PkgCmdPeekInd( U32 trd, U32 reg, U32 *val );

		//! Добавление команды чтение из прямого регистра
		void PkgCmdPeekDir( U32 trd, U32 reg, U32 *val );

		//! Выполние команды
		void PkgCmdExecute( void );
